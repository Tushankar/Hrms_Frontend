...
import { ArrowLeft, Download, Eye, Trash2, Eye as EyeIcon } from "lucide-react";
import toast, { Toaster } from "react-hot-toast";
import { Layout } from "../../Components/Common/layout/Layout";
import Navbar from "../../Components/Common/Navbar/Navbar";
import axios from "axios";

const PCAJobDescriptionSubmissions = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      const response = await axios.get(
        `${baseURL}/onboarding/hr-get-all-pca-job-description-submissions`,
  ...
  const handleClear = async (id) => {
    ...
    try {
      await axios.delete(
        `${baseURL}/onboarding/hr-clear-pca-submission/${id}`,
        { withCredentials: true }
      );
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-800 mb-2">
              PCA Job Description Submissions
            </h1>
            <p className="text-gray-600">
              Review all employee-submitted PCA Job Description documents
            </p>
          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
              <img
                src={
                  selectedSignature.startsWith("http")
                    ? selectedSignature
                    : `${baseURL}${
                        selectedSignature.startsWith("/")
                          ? selectedSignature
                          : `/${selectedSignature}`
                      }`
                }
                alt="Employee Signature"
                className="max-h-48 mx-auto block"
                style={{ maxWidth: "100%" }}
              />
            </div>
            <button
              onClick={() => setSelectedSignature(null)}
              className="mt-4 w-full px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400 transition-colors"
            >
              Close
            </button>
          </div>
...
export default PCAJobDescriptionSubmissions;
