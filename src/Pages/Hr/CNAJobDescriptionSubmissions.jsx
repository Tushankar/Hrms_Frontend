...
const CNAJobDescriptionSubmissions = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      const response = await axios.get(
        `${baseURL}/onboarding/hr-get-all-cna-job-description-submissions`,
  ...
  const handleClear = async (id) => {
    ...
    try {
      await axios.delete(
        `${baseURL}/onboarding/hr-clear-cna-submission/${id}`,
        { withCredentials: true }
      );
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-800 mb-2">
              CNA Job Description Submissions
            </h1>
            <p className="text-gray-600">
              Review all employee-submitted CNA Job Description documents
            </p>
          </div>
...
export default CNAJobDescriptionSubmissions;
