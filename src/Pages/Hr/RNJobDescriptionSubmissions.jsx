...
const RNJobDescriptionSubmissions = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      const response = await axios.get(
        `${baseURL}/onboarding/hr-get-all-rn-job-description-submissions`,
  ...
  const handleClear = async (id) => {
    ...
    try {
      await axios.delete(
        `${baseURL}/onboarding/hr-clear-rn-submission/${id}`,
        { withCredentials: true }
      );
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-800 mb-2">
              RN Job Description Submissions
            </h1>
            <p className="text-gray-600">
              Review all employee-submitted RN Job Description documents
            </p>
          </div>
...
export default RNJobDescriptionSubmissions;
