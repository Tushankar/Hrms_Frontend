...
const ServiceDeliveryPolicySubmissions = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      const response = await axios.get(
        `${baseURL}/onboarding/hr-get-all-service-delivery-policy-submissions`,
  ...
  const handleClear = async (id) => {
    ...
    try {
      await axios.delete(
        `${baseURL}/onboarding/hr-clear-service-delivery-policy-submission/${id}`,
        { withCredentials: true }
      );
        ...
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-800 mb-2">
              Service Delivery Policy Submissions
            </h1>
            <p className="text-gray-600">
              Review all employee-submitted Service Delivery Policy documents
            </p>
          </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {submissions.map((submission) => (
                    <tr key={submission._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-500">
                          {submission.employeeSignatureDate
                            ? new Date(
                                submission.employeeSignatureDate
                              ).toLocaleDateString()
                            : "-"}
...
export default ServiceDeliveryPolicySubmissions;
